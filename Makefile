# tui-containers — build, test and lint.
...
## demo: run against the in-memory sample machine.
demo:
...
screenshots: build
	python3 $(KIT)/tools/render-screenshots.py \
		...
		--screen main= --screen 'detail=\r' --screen 'logs=L' \
		--screen images=2 --screen system=4 \
		--screen help=?
